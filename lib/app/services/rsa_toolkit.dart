...
class RSAToolkit {
  ...
  Future<String> initRSAStrategy() async {
    if ((await this._safeZoneHandler.isPresent(key: 'publicKey')) &&
        (await this._safeZoneHandler.isPresent(key: 'privateKey'))) {
      return (await this._safeZoneHandler.readPublicKey());
    } else {
      return computeRSAKeyPair(getSecureRandom()).then((keyPair) {
        final AsymmetricKeyPair<PublicKey, PrivateKey> _keyPair = keyPair;

        final pubKey64 = encodePublicKeyToPemPKCS1(_keyPair.publicKey);
        final privKey64 = encodePrivateKeyToPemPKCS1(_keyPair.privateKey);

        this._safeZoneHandler.writePublicKey(pubKey64);
        this._safeZoneHandler.writePrivateKey(privKey64);

        this._safeZoneHandler.setRegStatus(false);
  ...
  Uint8List rsaEncrypt(String publicKey, Uint8List dataToEncrypt) {
    final RSAPublicKey _publicKey = parsePublicKeyFromPem(publicKey);
  ...
  Uint8List rsaDecrypt(String privateKey, Uint8List cipherText) {
    final RSAPrivateKey _privateKey = parsePrivateKeyFromPem(privateKey);
